...
python -c "import zipfile, os; files=['manifest.json','background.js','content.js','content.css','locales.js']; dirs=['icons','popup','pdf-viewer','whats-new']; packages=['magnifier-1.3.0.xpi','magnifier-1.3.0.zip','magnifier-debug.xpi','magnifier-debug.zip']; [(lambda z: [[z.write(f, f) for f in files if os.path.exists(f)], [z.write(os.path.join(r, fn), os.path.relpath(os.path.join(r, fn), '.')) for d in dirs for r, _, fl in os.walk(d) for fn in fl], z.close()])(zipfile.ZipFile(pkg, 'w', zipfile.ZIP_DEFLATED)) for pkg in packages]"
